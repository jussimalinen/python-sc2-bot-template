...
class CheeseBot(sc2.BotAI):
    ...
    def __init__(self):
        self.STEPS = [
            ...
            [19, self.build_depot],
            [19, self.build_ghosts(1)],
            [21, self.build_factory],
            [21, self.build_workers],
            [22, self.build_ghosts(1)],
            [24, self.build_workers],
            [24, self.build_depot],
            [26, self.build_workers],
            [26, self.build_factory_reactor],
            [26, self.build_depot],
            [27, self.build_cyclones(2)],
            [28, self.attaaaaack],
            [19, self.build_ghosts(1)],
            [28, self.build_cyclones(2)],
            [28, self.attaaaaack],
            [19, self.build_ghosts(1)],
            [1, self.build_cyclones(2)],
            [1, self.attaaaaack],
            [19, self.build_ghosts(1)],
            [1, self.build_cyclones(2)],
            [1, self.attaaaaack],
    ...
    def build_units(self, unit_type, build_with_type, total, condition=lambda: True, supply_used = 1):
        built_units = 0
        async def execute():
            nonlocal built_units
            build_with = self.units(build_with_type).ready
            if build_with.exists and condition() and self.can_afford(unit_type) and self.supply_left > supply_used:
                print('Building ' + str(unit_type))
                result = await self.do(build_with[0].train(unit_type))
                print(result)
                if not result:
                    built_units += 1
                    if built_units == total:
                        return True

        return execute

    def build_ghosts(self, total):
        return self.build_units(
                UnitTypeId.GHOST,
                UnitTypeId.BARRACKS,
                total,
                lambda: self.units(UnitTypeId.BARRACKS).exists and self.units(UnitTypeId.GHOSTACADEMY).ready.exists,
                2
        )

    def build_cyclones(self, total):
        return self.build_units(
                UnitTypeId.CYCLONE,
                UnitTypeId.FACTORY,
                total,
                lambda: self.units(UnitTypeId.FACTORYREACTOR).ready.amount > 0,
                3
        )